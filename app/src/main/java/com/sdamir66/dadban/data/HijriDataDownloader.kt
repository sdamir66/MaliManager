...
object HijriDataDownloader {
    ...
    /**
     * دانلود و ذخیره‌ی دیتای تقویم قمری برای یک سال جلالی
     */
    ...
    /**
     * تبدیل JSON به لیست HijriCache
     *
     * ساختار JSON:
     * {
     *   "1405": [
     *     {
     *       "header": { "qamari": "شوال - ذوالقعده - 1447", ... },
     *       "events": [...],
     *       "weeks": [{ "day": { "j": "1", "q": "02", "m": "21" }, "holiday": true }, ...]
     *     },
     *     ...
     *   ]
     * }
    ...
    private fun parseJson(jsonText: String, jalaliYear: Int): List<HijriCache> {
        ...
        for (monthIndex in 0 until monthsArray.length()) {
            ...
            // ═══ تجزیه‌ی qamari: "شوال - ذوالقعده - 1447" ═══
            val parts = qamariHeader.split("-").map { it.trim() }
            val hijriYear = parts.last().toIntOrNull() ?: 0
            val hijriMonths = parts.dropLast(1)   // ["شوال", "ذوالقعده"]

            val jMonth = monthIndex + 1   // ۱ = فروردین
            ...
            for (i in 0 until weeksArray.length()) {
                ...
                // ✅ اگه qDay == 1 و ماه قبلی تنظیم شده → ماه بعدی
