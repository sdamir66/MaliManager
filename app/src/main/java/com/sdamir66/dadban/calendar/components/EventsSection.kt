...
@Composable
private fun EventCard(event: Event, settings: CalendarSettings?) {
    // ✅ رنگ‌بندی بر اساس دسته (قابل تنظیم)
    val color = if (event.isHoliday) {
        parseColor(settings?.colorHoliday ?: "#E53935", DebitRed)
    } else {
        val hex = when (event.category) {
            EventCategory.RELIGIOUS -> settings?.colorReligious ?: "#9E9E9E"
            EventCategory.NATIONAL -> settings?.colorNational ?: "#9E9E9E"
            EventCategory.GLOBAL -> settings?.colorGlobal ?: "#9E9E9E"
